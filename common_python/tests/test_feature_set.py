...
SER_Y1 = SER_Y.apply(lambda v: 1 if v == CLASS else 0)
FEATURE1 = "feature1"
FEATURE2 = "feature2"
FEATURE3 = "feature3"
FEATURE_A = "Rv2009"
...
class TestFeatureVector(unittest.TestCase):
  ...
  def mkFeatureVector(self):
    features = [FEATURE_A, FEATURE_B]
    values = [-1, -1]
    dct = {f: v for f, v in zip(features, values)}
    return FeatureVector(dct)

  def testGetIndices(self):
    if IGNORE_TEST:
      return
    feature_vector = self.mkFeatureVector()
    indices = feature_vector.getIndices(DF_X)
